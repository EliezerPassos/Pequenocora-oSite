...
3. Verify the base project builds: `npm run build`. This project is **Vite + React (plain JS/JSX, no TypeScript) + Tailwind v4**, not Next.js/shadcn — there is no `src/app/`, no `next/font`, no shadcn primitives. Adapt every instruction below to this stack (see the "Project Conventions" note under each phase).
...
2. If they do, it's scroll-driven. Extract the mechanism: `IntersectionObserver`, `scroll-snap`, `position: sticky`, `animation-timeline`, or JS scroll listeners. *Project note:* prefer implementing the rebuilt version with `IntersectionObserver` over a raw `window.addEventListener('scroll', ...)` listener — a plain scroll listener fights with this project's Lenis smooth scroll and has previously caused the page to stutter/freeze (see `src/components/sections/StickyFeatureTabs.jsx` for a working reference implementation).
...
Every builder agent must verify `npm run lint` passes before finishing (this project has no TypeScript compiler step — `oxlint` is the static check). After merging worktrees, you verify `npm run build` passes. A broken build is never acceptable, even temporarily.
...
**Fonts** — Inspect `<link>` tags for Google Fonts or self-hosted fonts. Check computed `font-family` on key elements (headings, body, code, labels). Document every family, weight, and style actually used. *Project convention:* add the Google Fonts `<link>` tags to `index.html` (see the existing Baloo 2 / Nunito Sans / Poppins block there), then register the family as a CSS variable in the `@theme` block of `src/index.css` (e.g. `--font-display`, `--font-body`).

**Colors** — Extract the site's color palette from computed styles across the page. *Project convention:* this project has no shadcn tokens or `.dark` mode — colors live as custom properties in the `@theme` block of `src/index.css` (see the existing `--color-sun-*`, `--color-bloom-*`, `--color-ink-*`, `--color-cream-*` scales). Either extend those scales with the target site's actual shades, or add new named scales following the same `--color-<name>-<50-900>` pattern. Do not invent a shadcn-style `background`/`foreground`/`primary` naming scheme unless the user asks for one.

**Favicons & Meta** — Download favicons, apple-touch-icons, OG images, webmanifest to `public/seo/`. Update the `<head>` of `index.html` (title, meta description, theme-color, favicon `<link>`).

**Global UI patterns** — Identify any site-wide CSS or JS: custom scrollbar hiding, scroll-snap on the page container, global keyframe animations, backdrop filters, gradients used as overlays, **smooth scroll libraries** (Lenis, Locomotive Scroll — check for `.lenis`, `.locomotive-scroll`, or custom scroll container classes). *Project convention:* this project already uses **Lenis** for smooth scroll, wired in `src/hooks/useSmoothScroll.js` and applied once in `App.jsx` — do not add a second smooth-scroll library or a duplicate Lenis instance. Add any new global CSS/keyframes to `src/index.css` (outside the `@theme` block) and note any *other* libraries that need to be installed via `npm install`.
...
1. **Update fonts** in `index.html` (Google Fonts `<link>` tags) and register the family names as `--font-*` variables in the `@theme` block of `src/index.css`, to match the target site's actual fonts
2. **Update `src/index.css`** with the target's color tokens (as `--color-*` custom properties in `@theme`), spacing values, keyframe animations, utility classes, and any **global scroll behaviors** — remember Lenis is already wired via `src/hooks/useSmoothScroll.js`, don't duplicate it
3. **Create content data files** in `src/data/` (plain JS objects/arrays exported from a `.js` file, following the pattern of the existing `src/data/content.js` and `src/data/blog.js`) for the content structures you've observed. This project does not use TypeScript — skip interfaces/types entirely.
4. **Extract SVG icons** — find all inline `<svg>` elements on the page, deduplicate them, and save each as its own React component file under `src/assets/illustrations/` (one `.jsx` file per icon, following the pattern of the existing `Heart.jsx`, `Star.jsx`, `Sun.jsx`), then re-export them all from `src/assets/illustrations/index.js`. Name them by visual function (e.g., `SearchIcon`, `ArrowRightIcon`).
...
6. Verify: `npm run build` passes (and run `npm run lint` — this project uses `oxlint`, not TypeScript's compiler, as its static check)
...
5. **Identify assets** this section uses — which downloaded images/videos from `public/`, which icon components from `src/assets/illustrations/`. Check for **layered images** (multiple `<img>` or background-images stacked in the same container).
...
## Overview
- **Target file:** `src/components/sections/<ComponentName>.jsx` (or `src/components/ui/<ComponentName>.jsx` for a small reusable piece — follow this project's existing `sections/` vs `ui/` split, plain `.jsx`, no TypeScript)
...
- Icons used: <ArrowIcon>, <SearchIcon> from src/assets/illustrations/
...
- Which shared components to import (icons from `src/assets/illustrations/`, existing `src/components/ui/` primitives like `Container.jsx`, `Button.jsx`, `SectionHeading.jsx` — there is no shadcn here, reuse or extend these instead of inventing a parallel UI kit)
- The target file path (e.g., `src/components/sections/HeroSection.jsx`)
- Instruction to verify with `npm run lint` before finishing (no `tsc` — this project is plain JS)
...
After all sections are built and merged, wire everything together in the relevant page file under `src/pages/` (e.g. `src/pages/Home.jsx`), imported and routed from `src/App.jsx` via `react-router-dom` (already set up in this project — see `App.jsx` and `src/pages/Blog.jsx` for the pattern; add a new `<Route>` if the clone needs its own URL):
